...
- Le projet Vercel de l'API doit utiliser `apps/api` comme `Root Directory`, laisser l'`Output Directory` vide ; `apps/api/vercel.json` force `buildCommand: null` et `outputDirectory: null` sans `installCommand` personnalise car le hook standard `postinstall` gere la generation Prisma et la materialisation des dependances automatiquement ; `apps/api/package.json` ne doit pas exposer de script `build`
