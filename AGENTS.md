...
- La convention repo a conserver est volontairement stricte : un seul handler Vercel versionne pour l API (`apps/api/api/[[...route]].ts`) ; tout `404` de prod doit se corriger via la configuration Vercel du projet et du domaine, pas via des handlers de compatibilite supplementaires
