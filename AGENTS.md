...
- Le projet Vercel de l'API doit utiliser `apps/api` comme `Root Directory`, laisser l'`Output Directory` vide ; `apps/api/vercel.json` force `buildCommand: null`, `outputDirectory: null` et l'`installCommand` `pnpm install && pnpm vercel:prepare`, tandis que `apps/api/package.json` ne doit pas exposer de script `build`
...
- Les symlinks pnpm qui pointent vers le `node_modules` racine du monorepo ne sont pas consideres comme fiables pour le packaging Vercel de `apps/api` ; le script `apps/api/scripts/vercel/materialize-runtime-dependencies.cjs` copie donc physiquement `@axelys/legal` et `@prisma/client` dans `apps/api/node_modules` en environnement CI/Vercel ; le client Prisma genere est directement ecrit dans `apps/api/node_modules/.prisma/client` grace a l'output explicite dans `prisma/schema.prisma`, evitant ainsi les erreurs TypeScript sur les types Prisma manquants lors de la compilation Vercel
