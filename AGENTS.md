...
- Les symlinks pnpm qui pointent vers le `node_modules` racine du monorepo ne sont pas consideres comme fiables pour le packaging Vercel de `apps/api` ; le script `apps/api/scripts/vercel/materialize-runtime-dependencies.cjs` doit donc copier localement `@axelys/legal`, `@prisma/client` et `.prisma/client` dans `apps/api/node_modules` avant le bundling serverless
