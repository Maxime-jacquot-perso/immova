...
- Le deploiement Vercel de l'API passe desormais par `apps/api/api/index.ts` pour `/api` et `apps/api/api/[...route].ts` pour `/api/*` ; `src/main.ts` reste reserve au run local Nest classique
- Le projet Vercel de l'API doit utiliser `apps/api` comme `Root Directory`, laisser l'`Output Directory` vide ; `apps/api/vercel.json` force `buildCommand: null` et `outputDirectory: null` sans `installCommand` personnalise car le hook standard `postinstall` gere la generation Prisma et la materialisation des dependances automatiquement ; `apps/api/vercel.json` inclut aussi une regle `rewrites` pour router toutes les requetes `/api/*` vers le handler `/api` qui gere le routing NestJS et CORS ; `apps/api/package.json` ne doit pas exposer de script `build`
- La convention repo a conserver est volontairement stricte : uniquement les deux handlers Vercel attendus pour l API (`apps/api/api/index.ts` et `apps/api/api/[...route].ts`) ; tout `404` de prod doit se corriger via la configuration Vercel du projet, du domaine ou du deploiement, pas via des handlers de compatibilite supplementaires
- Les symlinks pnpm qui pointent vers le `node_modules` racine du monorepo ne sont pas consideres comme fiables pour le packaging Vercel de `apps/api` ; la generation du client Prisma et la materialisation des dependances utilisent maintenant le hook standard `postinstall` de npm/pnpm au lieu d'un `installCommand` personnalise ; le client Prisma est genere directement dans `apps/api/node_modules/.prisma/client` grace a l'output explicite dans `prisma/schema.prisma`, puis le script `apps/api/scripts/vercel/materialize-runtime-dependencies.cjs` copie physiquement `@axelys/legal` et `@prisma/client` en environnement CI/Vercel, evitant ainsi les erreurs TypeScript sur les types Prisma manquants lors de la compilation Vercel ; le package `@axelys/legal` utilise un hook `prepare` pour se builder automatiquement avant le `postinstall` de l'API
